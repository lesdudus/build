...
The **Timer** navigation item opens a standalone, silent interval timer in every layout. Set exercise duration, rest between sets (zero allowed), and 1-99 sets. The planned duration includes a five-second preparation countdown and excludes rest after the final set. Each phase may be up to 60 minutes. Valid settings are remembered on this browser.
...
Timing derives from elapsed timestamps rather than counting callbacks. If the browser suspends updates while backgrounded, returning catches up to the correct phase; this is not a promise of background execution. Changing the device's wall clock can affect elapsed time. Timer snapshots are saved locally about once a second and on visibility/page-exit events. After reload, **Resume**, **Restart**, or **Discard** is required: a recovered timer stays paused at its last saved position and does not count time while the page was closed. Abrupt termination may lose the final unsaved fraction. These non-sensitive timer settings/recovery keys are device-wide and are not cloud-synced. Storage failures are shown without disabling the in-memory timer.

The timer works offline after the production app shell is cached. Responsive screenshots and automated Edge checks cover 320/390-pixel phones, landscape, desktop, native fullscreen/fallback, denied/mocked wake locks, phase transitions, pause, restart/end confirmations, zero rest, one/multiple sets, background-time catch-up and offline interrupted recovery. Physical iOS/Android fullscreen, wake-lock and lock-screen behavior remain unverified. Browser and OS controls always take precedence. After updating the app, close all its tabs and reopen it if the existing service worker still shows the previous version.
